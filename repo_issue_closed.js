...
let username = "anveshcv@umich.edu";
...
fs.createReadStream("APIsName.csv")
    ...
    .on('end', function () {
        ...
        setTimeout(apiInformation.bind(this, a), 3000);
    });
                    ...
                    if (i <= no_of_pages) {
                        ...
                        function getRepoDetailsByIndex(i) {
                            if (i < repositories.length) {
                                ...
                                var options = {
                                    host: 'api.github.com',
                                    path: '/repos/' + repositories[i].rep_name + '/issues?state=closed',
                                    method: 'GET',
                                    headers: {
                                        'user-agent': 'node.js',
                                        "Authorization": auth
                                ...
                                var request = https.request(options, function (resp) {
                                    ...
                                    resp.on("end", () => {
                                        ...
                                        if (parsed == null) {
                                            no_of_closed_issues_pages = 1;
                                        }
                                        //console.log(data);
                                        // console.log(res.headers);
                                        else {
                                            no_of_closed_issues_pages = parsed.last.page;
                                        }
                                        function getClosedComments(p) {
                                            if (p <= no_of_closed_issues_pages) {
                                                var options2 = {
                                                    host: 'api.github.com',
                                                    path: '/repos/' + repositories[i].rep_name + '/issues?state=closed&page=' + p,
                                                    method: 'GET',
                                                    headers: {
                                                        'user-agent': 'node.js',
                                                        "Authorization": auth
                                                ...
                                                console.log("Fetching closed issues of page "+ p +" of "+ repositories[i].rep_name + "(" + apiNames[a].ID + ")");
                                                https.get(options2, (resp1) => {
                                                    let data = "";
                                                    resp1.on("data", chunk => {
                                                        data += chunk;
                                                    });
                                                    resp1.on("end", () => {
                                                        ...
                                                        getCommentsByIssue(0, activity_data, function () {
                                                            ...
                                                            p++;
                                                            setTimeout(getClosedComments.bind(this,p),3000);
                                                ...
                                                const csvWriter = createCsvWriter({
                                                    path: 'CSV files/Repository closed issues information/' + apiNames[a].ID + '.csv',
                                                    header: [
                                                        ...
                                                        { id: 'issue_state', title:'Issue State'},
                                                        ...
                                                        { id: 'issue_comment_time_diff', title: 'Issue Comment Time Difference (dd:hh:mm:ss)'},
                                                        { id: 'issue_comment_updation_time_diff', title: 'Issue comment Updation Time ifference (dd:hh:mm:ss)'}
                                                    ]
                                                });
                                                ...
                                                i++;
                                                setTimeout(getRepoDetailsByIndex.bind(this, i), 3000);
                                            }
                                        }
                                        getClosedComments(1)
                                    });
                                            ...
                                            res.on('end', () => {
                                                ...
                                                if (comments_info.length == 0) {
                                                    var resultWithoutComments = {
                                                        ...
                                                        issue_closed_at: activity_data[y].closed_at ? activity_data[y].closed_at : "Not yet closed",
                                                        issue_updation_time_diff: issueUpdationTimeDiff,
                                                        issue_label: label_value,
                                                        issue_reaction: reaction_type,
                                                        issue_state:activity_data[y].state
                                                    }
                                                    ...
                                                    for (c = 0; c < comments_info.length; c++) {
                                                        ...
                                                        var IssuecommentTime = moment(commentCreatedTime, "YYYY-MM-DD'T'HH:mm:ss:SSSZ").diff(moment(issueCreatedTime, "YYYY-MM-DD'T'HH:mm:ss:SSSZ"));
                                                        var icT = moment.duration(IssuecommentTime);
                                                        var issueCommentCreatedTimeDiff= icT.format("dd:hh:mm:ss");
                                                        
                                                        var IssuecommentUpdationTime = moment(commentUpdatedTime, "YYYY-MM-DD'T'HH:mm:ss:SSSZ").diff(moment(issueCreatedTime, "YYYY-MM-DD'T'HH:mm:ss:SSSZ"));
                                                        var icuT = moment.duration(IssuecommentUpdationTime);
                                                        var issueCommentUpdationTimeDiff= icuT.format("dd:hh:mm:ss");
                                                        ...
                                                        var resultWithComments = {
                                                            ...
                                                            issue_comment_time_diff: issueCommentCreatedTimeDiff,
                                                            issue_comment_updation_time_diff: issueCommentUpdationTimeDiff
                                                        }
                                                ...
                                                y++;
                                                setTimeout(getCommentsByIssue.bind(this, y, activity_data, onSuccess), 3000);
                                            });
